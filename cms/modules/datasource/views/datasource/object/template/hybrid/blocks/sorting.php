...
$order_fields = array();
foreach ($object->doc_order as $data)
{
...
foreach ($object->doc_order as $data)
{
	if(isset($ids[key($data)]))
		$selected_fields[key($data)] = (($data[key($data)] == Datasource_Object_Decorator::ORDER_ASC) ? '+' : '-') .' '. $ids[key($data)];
}
