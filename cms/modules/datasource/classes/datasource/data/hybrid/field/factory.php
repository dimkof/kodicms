...
class DataSource_Data_Hybrid_Field_Factory {

	/**
	 * 
	 * @param DataSource_Data_Hybrid_Record $record
	 * @param DataSource_Data_Hybrid_Field $field
	 * @return boolean
	 */
	public static function create_field( 
			DataSource_Data_Hybrid_Record $record, DataSource_Data_Hybrid_Field $field) 
	{
	...
	/**
	 * 
	 * @param DataSource_Data_Hybrid_Field $old
	 * @param DataSource_Data_Hybrid_Field $new
	 * 
	 * @return DataSource_Data_Hybrid_Field
	 */
	public static function update_field($old, $new) 
	{
		$ds = DataSource_Data_Hybrid_Field_Utils::load_ds($old->ds_id);
	 ...
	 * 
	 * @param integer $id
	 * @return null|DataSource_Data_Hybrid_Field
	 */
	...
	protected static function _get_field_from_array($r) 
	{
		...
		$class_name = 'DataSource_Data_Hybrid_Field_' . $r['family'];
		...
		$result = DataSource_Data_Hybrid_Field::factory($r['family'], $r);
	...
	{
		$key = str_replace(DataSource_Data_Hybrid_Field::PREFFIX, '', $key);
		$key = URL::title($key, '_');
		$key = strtolower($key);
		...
		return DataSource_Data_Hybrid_Field::PREFFIX . $key;
	}
